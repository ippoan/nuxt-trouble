...
generated-from: nuxt-trouble:6e3a5d0e
paths: [app/, server/]
...
- **経過記録の並び替えは `reorderTasks` 一括採番** (Refs #240): `moveTask(from, to)` が表示順の `task_id` を並べ替えて全件 `PUT /api/trouble/tickets/{id}/tasks/reorder` に渡し、応答 (更新後の一覧) をそのまま `tasks` に入れる。**隣接 2 行の `sort_order` を交換する形に戻してはいけない** — 既存データは `sort_order` が全行 0 なので「0 と 0 の交換」になり、PUT が 200 でも並びが動かない (テストのフィクスチャに 0/1/2 を持たせると再現しないので、回帰テストは全行 0 のデータで書く)。採番は rust-alc-api 側 (migration 127 + reorder endpoint)。
