...
generated-from: nuxt-trouble:28fc12b6a8f57a90bfc7f2ecfeb1e3620796318f
paths: [app/, server/]
...
- **`TicketTaskList.vue` の 1 件編集モーダル** (Refs #191): 各行の鉛筆ボタンで開く。全フィールド編集 + Alt+↓/Alt+↑ で前後行に移動 (window keydown listen、端では停止)。未保存変更は Alt 移動時に自動保存 (失敗時は移動しない)、キャンセルは破棄。`assigned_to` は employee 名 ⇄ id 変換 (追加フォームと同じ)。行 grid は 10 列 (`GRID` 定数) — 列を増減したら Row1/Row2 両方の span 数も合わせる。
